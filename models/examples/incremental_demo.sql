MODEL (
  ...
  kind INCREMENTAL_BY_TIME_RANGE (
    time_column (event_timestamp, '%Y-%m-%d'), -- How does this behave? DELETE by time range, then INSERT
  ...
  start '2024-06-17', --This is as far back I want to care about backfilling my data
...
product_usage AS (
  SELECT
    ...
    user_segment
  FROM raw_sources.product_usage  -- Source B
  WHERE last_usage_date BETWEEN DATE_SUB(@start_ds, INTERVAL 30 DAY) AND @end_ds  -- Include recent usage data
),
