...
def raw_event_loader(mock_bigquery_client):
    with patch('demo_scripts.load_raw_events.bigquery.Client', return_value=mock_bigquery_client):
...
def test_generate_fake_data(raw_event_loader):
    ...
    for row in data:
        assert set(row.keys()) == {'event_id', 'event_name', 'event_timestamp', 'user_id'}
...
def test_append_to_bigquery_table(raw_event_loader, mock_bigquery_client):
    with patch.object(raw_event_loader, 'generate_fake_data') as mock_generate_data, \
         patch.object(raw_event_loader, 'create_table_if_not_exists') as mock_create_table:
        
        mock_generate_data.return_value = [{'event_id': '1', 'event_name': 'test', 'event_timestamp': datetime.now(), 'user_id': 'user1'}]
        
        raw_event_loader.append_to_bigquery_table('test_dataset.test_table', 1, '2023-04-15')
