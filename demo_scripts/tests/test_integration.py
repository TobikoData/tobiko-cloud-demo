...
def test_integration_append_and_rename(mock_bq_client, mock_credentials, mock_get_sa_info, runner, mock_bigquery_client):
    ...
    # Append raw data
    result = runner.invoke(app, ['append-rawdata', '--num-rows', '10', '--table-name', 'test_dataset.test_table'])
    assert result.exit_code == 0
    mock_bigquery_client.load_table_from_dataframe.assert_called_once()

    # Rename column
    result = runner.invoke(app, ['rename-column', '--old', 'event_name', '--new', 'renamed_event'])
    assert result.exit_code == 0
...
def test_integration_error_handling(mock_bq_client, mock_credentials, mock_get_sa_info, runner, mock_bigquery_client):
    ...
    mock_bigquery_client.load_table_from_dataframe.side_effect = Exception("Failed to append data")
