...
def test_gen_surrogate_key_error_single_field():
    ...
    assert "List should have at least 2 items after validation, not 1" in str(excinfo.value)

def test_gen_surrogate_key_error_empty_list():
    with pytest.raises(ValueError) as excinfo:
        gen_surrogate_key(evaluator=MacroEvaluator, field_list=[])
    
    assert "List should have at least 2 items after validation, not 0" in str(excinfo.value)